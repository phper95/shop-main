...
import (
	...
	"shop/app/models/vo"
	"shop/app/params"
	"shop/app/service/cart_service"
	cartVo "shop/app/service/cart_service/vo"
	orderDto "shop/app/service/order_service/dto"
	ordervo "shop/app/service/order_service/vo"
	userVO "shop/app/service/wechat_user_service/vo"
	"shop/pkg/constant"
...
func (d *Order) CancelOrder() error {
	...
	//删除订单
	err = tx.Where("id = ?", order.Id).Delete(&models.shopStoreOrder{}).Error
	if err != nil {
		global.LOG.Error(err)
...
func (d *Order) OrderComment() error {
	...
	var replys []models.shopStoreProductReply
	for _, data := range d.ReplyParam {
		reply := models.shopStoreProductReply{
			Uid:          d.Uid,
			Oid:          order.Id,
	...
	err = tx.Model(&models.shopStoreProductReply{}).Select("uid", "oid", "product_id",
		"product_score", "service_score", "comment", "pics", "unique").Create(replys).Error
	if err != nil {
		global.LOG.Error(err)
		return errors.New("评价失败-0001")
	}
	err = tx.Model(&models.shopStoreOrder{}).Where("id = ?", order.Id).Update("status", 3).Error
	if err != nil {
		global.LOG.Error(err)
...
func (d *Order) TakeOrder() error {
	...
	//修改订单状态
	err = tx.Model(&models.shopStoreOrder{}).Where("id = ?", order.Id).Update("status", 2).Error
	//增加状态
...
//创建订单
func (d *Order) CreateOrder() (*models.shopStoreOrder, error) {
	var err error
	tx := global.Db.Begin()
	defer func() {
		if err != nil {
	...
	var (
		userAddress models.shopUserAddress
		totalNum    = 0
		cartIds     = make([]string, 0)
		//integral = 0
		gainIntegral = 0
	)
	err = global.Db.Model(&models.shopUserAddress{}).
		Where("uid = ?", d.Uid).
	...
	storeOrder := &models.shopStoreOrder{
		Uid:            d.Uid,
	...
	err = tx.Model(&models.shopStoreOrder{}).
	...
	//todo 扣积分
	//todo 消费优惠券
	orderInfoList := make([]models.shopStoreOrderCartInfo, 0)
	//减库存加销量
	for _, vo := range cartInfo {
		...
		b, _ := json.Marshal(vo)
		uuid := ksuid.New()
		orderCartInfo := models.shopStoreOrderCartInfo{
			Oid:          storeOrder.Id,
	...
	//保存购物车商品信息
	err = tx.Model(&models.shopStoreOrderCartInfo{}).Create(orderInfoList).Error
	if err != nil {
...
func (d *Order) GetOrderInfo() (*ordervo.StoreOrder, error) {
	...
	err := global.Db.Model(&models.shopStoreOrder{}).
...
func HandleOrder(order *ordervo.StoreOrder) *ordervo.StoreOrder {
	var (
		orderInfoList []models.shopStoreOrderCartInfo
		cart          cartVo.Cart
		statusDto     orderDto.Status
	)
	global.Db.Model(&models.shopStoreOrderCartInfo{}).Where("oid = ?", order.Id).Find(&orderInfoList)
...
func (d *Order) Check() (map[string]interface{}, error) {
	...
	var order *models.shopStoreOrder
	error := global.Db.Model(&models.shopStoreOrder{}).
		Where("`unique` = ?", d.Key).
...
func (d *Order) ConfirmOrder() (*ordervo.ConfirmOrder, error) {
	...
	//获取默认地址
	global.Db.Model(&models.shopUserAddress{}).
		Where("uid = ?", d.Uid).
		Where("is_default = ?", 1).
		First(&userAddress)
...
func getOrderPriceGroup(cartInfo []cartVo.Cart, userAddress *models.shopUserAddress) orderDto.PriceGroup {
	var (
...
func (d *Order) Deliver() error {
	...
	var express models.shopExpress
	err := global.Db.Model(&models.shopExpress{}).Where("name = ?", d.M.DeliveryName).First(&express).Error
	if err != nil {

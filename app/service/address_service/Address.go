...
func (d *Address) DelAddress() error {
	err := global.Db.
		Where("uid = ?", d.Uid).
		Where("id = ?", d.Id).
		Delete(&models.shopUserAddress{}).Error
	if err != nil {
...
func (d *Address) SetDefault() error {
	...
	err = tx.Model(&models.shopUserAddress{}).
...
//get list
func (d *Address) GetList() ([]models.shopUserAddress, int, int) {
...
//get city list
func (d *Address) GetCitys() []models.shopSystemCity {
	key := constant.CITY_LIST
	if b, err := redis.Get(key); err == nil {
		var city []models.shopSystemCity

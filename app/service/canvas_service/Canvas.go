...
type Canvas struct {
	...
	M *models.shopStoreCanvas

	Ids []int64
}

func (d *Canvas) Get() vo.ResultList {
	var data models.shopStoreCanvas
	err := global.Db.Model(&models.shopStoreCanvas{}).Where("terminal = ?", d.Terminal).First(&data).Error
	if err != nil {
...
func (d *Canvas) Save() error {
	if d.M.Id == 0 {
		...
		data := &models.shopStoreCanvas{
			Name:     d.M.Name,
			Terminal: d.M.Terminal,
			Json:     d.M.Json,
		}
		return global.Db.Model(&models.shopStoreCanvas{}).Where("id = ?", d.M.Id).Updates(data).Error
	}

...
import (
	...
	"shop/app/service/product_service"
	productVo "shop/app/service/product_service/vo"
...
func (d *Cart) ChangeCartNum() error {
	var (
		cart models.shopStoreCart
		err  error
	)
	err = global.Db.Model(&models.shopStoreCart{}).
		Where("uid = ?", d.Uid).
...
func (d *Cart) GetCartList() map[string]interface{} {
	...
	//}
	var (
		carts   []models.shopStoreCart
		valid   = make([]cartVo.Cart, 0)
		invalid = make([]cartVo.Cart, 0)
		error   error
	)
	error = global.Db.Model(&models.shopStoreCart{}).
	...
	for _, cart := range carts {
		...
		if err != nil {
			global.LOG.Error(err)
			global.Db.Where("id = ?", cart.Id).Delete(&models.shopStoreCart{})
			continue
		}
		...
		global.Db.Model(&models.shopStoreProductAttrValue{}).
...
func (d *Cart) AddCart() (int64, error) {
	...
	var cart models.shopStoreCart
	whereCart := &models.shopStoreCart{
		Uid:               d.Uid,
	...
	if err != nil {
		...
		err = global.Db.Model(&models.shopStoreCart{}).Create(whereCart).Error
		if err != nil {

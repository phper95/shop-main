...
import (
	...
	"gorm.io/gorm"
	"shop/app/models"
	"shop/app/service/order_service"
	orderDto "shop/app/service/order_service/dto"
	"shop/app/service/wechat_user_service"
...
func PaySuccess(tx *gorm.DB, orderId, payType string) error {
	...
	//修改订单状态
	updateOrder := &models.shopStoreOrder{
		Paid:    orderEnum.PAY_STATUS_1,
		PayType: payType,
		PayTime: time.Now(),
	}
	err = tx.Model(&models.shopStoreOrder{}).Where("order_id = ?", orderId).Updates(updateOrder).Error

...
type shopStoreProductAttrValue struct {
	ID           int64   `json:"id"`
...
func (shopStoreProductAttrValue) TableName() string {
	return "shop_store_product_attr_value"
}

func GetAttrValueByProductIdAndSku(productId int64, sku string) *shopStoreProductAttrValue {
	var attrValue shopStoreProductAttrValue
...
func AddProductttrValue(attrs []dto2.ProductFormat, productId int64) error {
	...
	var valueGroup []shopStoreProductAttrValue
	for _, val := range attrs {
		...
		uuid := ksuid.New()
		var storeProductAttrValue = shopStoreProductAttrValue{
			ProductId:    productId,
			Sku:          str,
			Price:        price,
			Cost:         cost,
...
func DelByProductttrValue(productId int64) (err error) {
	err = db.Where("product_id = ?", productId).Delete(shopStoreProductAttrValue{}).Error
	return err
}

...
type shopStoreProduct struct {
	...
	ProductCate  *shopStoreCategory `json:"productCate" gorm:"foreignKey:CateId;association_autoupdate:false;association_autocreate:false"`
...
func (shopStoreProduct) TableName() string {
	return "shop_store_product"
}

func GetProduct(id int64) shopStoreProduct {
	var product shopStoreProduct
...
// get all
func GetFrontAllProduct(pageNUm int, pageSize int, maps interface{}, order string) (int64, []shopStoreProduct) {
	var (
		total int64
		data  []shopStoreProduct
	)

	db.Model(&shopStoreProduct{}).Where(maps).Count(&total)
	if order == "" {
		order = "id desc"
	}
...
// get all
func GetAllProduct(pageNUm int, pageSize int, maps interface{}) (int64, []shopStoreProduct) {
	var (
		total int64
		data  []shopStoreProduct
	)
...
func AddProduct(m *shopStoreProduct) error {
	var err error
...
func UpdateByProduct(id int64, m *shopStoreProduct) error {
	var err error
	err = db.Model(&shopStoreProduct{}).Where("id = ?", id).Updates(m).Error
	if err != nil {
		return err
	}
...
func OnSaleByProduct(id int64, status int) (err error) {
	...
	err = db.Model(&shopStoreProduct{}).Where("id = ?", id).Update("is_show", isShow).Error
	return
}

func DelByProduct(ids []int64) error {
	var err error
	err = db.Where("id in (?)", ids).Delete(&shopStoreProduct{}).Error
	if err != nil {
		return err
	}

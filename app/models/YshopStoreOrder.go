...
func (shopStoreOrder) TableName() string {
	return "shop_store_order"
}

func GetAllOrder(pageNUm int, pageSize int, maps interface{}) (int64, []shopStoreOrder) {
	var (
		total int64
		data  []shopStoreOrder
	)

	db.Model(&shopStoreOrder{}).Where(maps).Count(&total)
	db.Where(maps).Offset(pageNUm).Limit(pageSize).Order("id desc").Find(&data)

	return total, data
}

func GetAdminAllOrder(pageNUm int, pageSize int, maps interface{}) (int64, []shopStoreOrder) {
	var (
		total int64
		data  []shopStoreOrder
	)
...
func AddStoreOrder(m *shopStoreOrder) error {
	var err error
...
func UpdateByStoreOrder(m *shopStoreOrder) error {
	var err error
...
func DelByStoreOrder(ids []int64) error {
	var err error
	err = db.Where("id in (?)", ids).Delete(&shopStoreOrder{}).Error
	if err != nil {
		return err
	}

...
type shopStoreProductAttr struct {
	Id         int64  `json:"id"`
...
func (shopStoreProductAttr) TableName() string {
...
func AddProductAttr(items []dto2.FormatDetail, productId int64) error {
	...
	var attrGroup []shopStoreProductAttr
	for _, val := range items {
		detailStr := strings.Join(val.Detail, ",")
		var storeProductAttr = shopStoreProductAttr{
			ProductId:  productId,
			AttrName:   val.Value,
...
func DelByProductttr(productId int64) (err error) {
	err = db.Where("product_id = ?", productId).Delete(shopStoreProductAttr{}).Error
	return err
}
